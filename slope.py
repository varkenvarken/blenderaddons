...
class Slope2VGroup(bpy.types.Operator, Slope):
    ...
    low: bpy.props.FloatProperty(
        name="Lower limit",
        description="Angles smaller than this get a unit weight",
        subtype="ANGLE",
        default=0,
        max=pi,
        min=0,
    ...
    high: bpy.props.FloatProperty(
        name="Upper limit",
        description="Angles larger than this get a zero weight",
        subtype="ANGLE",
        default=pi / 2,
        max=pi,
        min=0.01,
    ...
    power: bpy.props.FloatProperty(
        name="Power", description="Shape of mapping curve", default=1, min=0, max=10
    )
    mirror: bpy.props.BoolProperty(
        name="Mirror", description="Limit angle to 90 degrees", default=False
    )
    worldspace: bpy.props.BoolProperty(
        name="World space",
        description="Use world space instead of object space coordinates",
        default=False,
    )
    ...
    def poll(self, context):
        p = (
            ...
            and isinstance(context.active_object, bpy.types.Object)
...
class Slope2VCol(bpy.types.Operator, Slope):
    ...
    curve: bpy.props.BoolProperty(
        name="Use brush curve",
        description="Apply brush curve after calculculating values",
        default=False,
    )
    normal: bpy.props.BoolProperty(
        name="Map normal",
        description="Convert face normal to vertex colors instead of slope angle",
        default=False,
    )
    worldspace: bpy.props.BoolProperty(
        name="World space",
        ...
        for poly in mesh.polygons:
            for loop_index in range(poly.loop_start, poly.loop_start + poly.loop_total):
                ...
                if self.normal:
                    ...
                    vertex_colors[loop_index].color = [
                        (pnormal.x + 1) / 2,
                        (pnormal.y + 1) / 2,
                        (pnormal.z + 1) / 2,
                        1,
                    ...
                    if self.curve:
                        weight = bcurvemap.evaluate(bcurve, 1.0 - weight)
                    vertex_colors[loop_index].color = [weight, weight, weight, 1.0]
    ...
    def draw(
        self, context
    ):  # provide a draw function here to show use brush option only with versions that have the new initialize function
        layout = self.layout
        ...
        if not self.normal:
            ...
            if self.curve:
                layout.label(
                    text="Run Paint -> Slope again\n after changing brush curve!"
                )


def menu_func_weight(self, context):
    self.layout.operator(Slope2VGroup.bl_idname, text="Slope", icon="PLUGIN")


def menu_func_vcol(self, context):
    self.layout.operator(Slope2VCol.bl_idname, text="Slope", icon="PLUGIN")
