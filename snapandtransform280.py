...
#  (c) 2017 - 2019 Michel J. Anders (varkenvarken)
#
...
class OriginToLowest(bpy.types.Operator):
	...
	def execute(self, context):
		...
		if count > 0:  # degenerate mesh, but better safe than sorry
			...
			context.scene.cursor.location = min_co
			bpy.ops.object.origin_set(type='ORIGIN_CURSOR')
		if context.mode == 'EDIT_MESH':
			bpy.ops.object.editmode_toggle()
		return {'FINISHED'}
...
def register():
	...
	bpy.utils.register_class(OriginToLowest)
...
def unregister():
	...
	bpy.utils.unregister_class(OriginToLowest)
