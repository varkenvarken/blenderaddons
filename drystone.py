...
#  (c) 2014,2025 Michel J. Anders (varkenvarken)
#
...
from random import random, seed as rseed, randint, choice, sample
...
def vertical_t_joint(vert):
    if len(vert.link_edges) == 3:
        ...
        v1 = vertical(e1)
        v2 = vertical(e2)
        if not (v0 or v1) and (edge_angle(vert, e0, e1) > 0.97 * pi):
            return True
        if not (v1 or v2) and (edge_angle(vert, e1, e2) > 0.97 * pi):
            return True
        if not (v2 or v0) and (edge_angle(vert, e2, e0) > 0.97 * pi):
            return True
    return False
...
def non_gap(edge):
    v0 = edge.verts[0]
    v1 = edge.verts[1]
    return (fully_connected(v0) or vertical_t_joint(v0)) and (
        fully_connected(v1) or vertical_t_joint(v1)
    )
...
def get_movable_edges(bm):
    ...
    for edge in bm.edges:
        if (
            vertical(edge)
            and vertical_t_joint(edge.verts[0])
            and vertical_t_joint(edge.verts[1])
        ):
            s.append(edge)
    return s
...
class DryStone(bpy.types.Operator):
    ...
    xsub: IntProperty(
        name="X Subdividions",
        description="Number of subsivisions in the x direction",
        min=4,
        soft_max=100,
        default=30,
    )
    ysub: IntProperty(
        name="Y Subdividions",
        description="Number of subsivisions in the y direction",
        min=4,
        soft_max=100,
        default=10,
    )
    nv: IntProperty(
        name="Quad blocks",
        description="Number of big square blocks",
        min=0,
        soft_max=100,
        default=15,
    )
    ne: IntProperty(
        name="Long blocks",
        description="Number of longer blocks",
        min=0,
        soft_max=200,
        default=100,
    )
    seed: IntProperty(name="Random Seed", description="Random Seed", min=0, default=0)
    randomedge: FloatProperty(
        name="Random Edge",
        description="Randomize movable edges",
        min=0.0,
        max=0.5,
        default=0.3,
    )
    randomvert: FloatProperty(
        name="Random Vert",
    ...
    zrandom: FloatProperty(
        name="Random Displacement",
        description="Randomize block depth",
        min=0.0,
        max=0.5,
        default=0.01,
    )
    randomuv: BoolProperty(
        name="Randomize UVs",
        description="Randomize the uv-offset of individual stones",
        default=True,
    )
    ...
    def execute(self, context):
        ...
        bpy.ops.mesh.primitive_grid_add(
            x_subdivisions=self.xsub, y_subdivisions=self.ysub, enter_editmode=True
        )
        ...
        for vert in bm.verts:
            ...
            y = (
                (noise(vert.co + Vector((11.1, 13.12, 17.14))) * 2 - 1)
                * self.randomvert
                * (2.0 / self.ysub)
                * 0.5
            )
        ...
        for face in bm.faces:
            offset = Vector((random(), random())) if self.randomuv else Vector((0, 0))
            for loop in face.loops:
                loop_uv = loop[uv_layer]
                coords = loop.vert.co.xy
                loop_uv.uv = coords + offset
        ...
        for poly in me.polygons:
            offset = (
                Vector((random(), random(), 0)) if self.randomuv else Vector((0, 0, 0))
            )
